...
def run_awb_method_2(img, cfg: AWBConfig):
    """
    ...
    # Recompute a boolean for empty regions prior to the count clamping above
    ...
    points = region_illuminants[valid] 

    if points.shape[0] < 2 or np.allclose(points, points[0]):
        cl_centers = np.vstack([points[0], points[0]])
        cl_labels_valid = np.zeros(points.shape[0], dtype=int)
    else:
        cl_labels_valid, cl_centers = cluster.cluster(
            points,
    ...
    cl_labels[valid] = cl_labels_valid
    T.toc()  # Clustering
    ...
    if cfg.downsample and scale != 1.0:
        smoothed_gain_map_out = cv2.resize(smoothed_gain_map, (orig_w, orig_h), interpolation=cv2.INTER_LINEAR)
        ...
        if cfg.fast:
            gain_map_out = None
            smoothed_gain_map_out = None
            mask_out = None
        else:
            smoothed_gain_map_out = smoothed_gain_map
            mask_out = mask
    ...
    np.multiply(img, smoothed_gain_map_out, out=img_awb)
    ...
    return {
        ...
        'smoothed_gain_map': smoothed_gain_map_out,
